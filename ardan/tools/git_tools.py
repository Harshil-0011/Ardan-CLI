...
def git_branch(name: str, cwd: str = ".") -> ToolResult:
    try:
        subprocess.run(["git", "checkout", "-b", name], cwd=cwd, check=True, capture_output=True)
...
def git_status(cwd: str = ".") -> ToolResult:
    try:
        res = subprocess.run(["git", "status"], cwd=cwd, check=True, capture_output=True, text=True)
...
def generate_commit_message(cwd: str = ".") -> ToolResult:
    ...
    return ToolResult(True, "Agent instruction: run 'git diff', analyze it, and write a commit message.", "")
