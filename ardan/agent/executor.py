...
from ardan.tools.file_tools import (
    read_file,
    write_file,
    append_file,
    list_files,
...
from ardan.tools.shell_tools import (
    run_command,
    run_script,
    compile_c,
    compile_cpp,
...
class Executor:
    ...
    async def execute_step(
        self, step: Dict[str, Any], max_turns: int = 5
    ) -> AsyncIterator[str]:
        messages = [
        ...
        for turn in range(max_turns):
            response_full = ""
            async for chunk in failover_generate(
                messages, GenerationConfig(), self.provider, self.settings
            ):
                response_full += chunk
                yield chunk
            ...
            for tc_str in tool_calls:
                try:
                    ...
                    if name in self.tools:
                        ...
                        if res.success:
                            if name == "write_file":
                                self.memory.files_created.append(args.get("path"))
                            if name in ["append_file", "search_and_replace"]:
                                self.memory.record_modification(args.get("path"))
                            if name in ["run_command", "run_script"]:
                                self.memory.commands_run.append(
                                    args.get("command") or "script"
                                )
                    else:
                        messages.append(
                            Message(
                                role="user", content=f"Error: Tool {name} not found."
                            )
                        )
                except Exception as e:
                    messages.append(
                        Message(
                            role="user", content=f"Error parsing tool call: {str(e)}"
                        )
                    )
