...
class Memory:
    def __init__(self, workspace: str, session_id: Optional[int] = None):
        ...
        if session_id:
            ...
            self.session_file = os.path.join(
                self.session_dir, f"session_{self.session_id}.json"
            )
            self._load()
        else:
            ...
            sessions = sorted(
                [
                    s
                    for s in os.listdir(self.session_dir)
                    if s.startswith("session_") and s.endswith(".json")
                ]
            )
            if sessions:
    ...
    async def summarize(self, provider: Any):
        ...
        msgs = [
            Message(
                role="system",
                content="Summarize the following agent session history into a concise technical summary.",
            ),
        ...
        self.history = [
            {"timestamp": time.time(), "type": "SUMMARY", "content": summary}
        ]
        self._save()
