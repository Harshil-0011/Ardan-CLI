...
def detect_active_provider(cli_flag: Optional[str], env_var: Optional[str], config_val: str) -> str:
    if cli_flag: return cli_flag
    if env_var: return env_var
    return config_val or "ollama"

async def failover_generate(messages: List[Any], config: Any, primary_provider: BaseProvider, settings: Any):
