...
class OpenAIProvider(BaseProvider):
    ...
    supported_models = ["gpt-4.1", "gpt-4.1-mini", "gpt-4o", "gpt-4o-mini", "o3", "o4-mini"]
    ...
    async def generate(self, messages: List[Message], config: GenerationConfig) -> AsyncIterator[str]:
        if not self.client:
            raise ArdanProviderError("OpenAI SDK not installed or API key missing. Install with: pip install ardan[openai]", self.name)
    ...
    async def list_models(self) -> List[ModelInfo]:
        return [
            ModelInfo(id="gpt-4.1", name="GPT-4.1", provider=self.name, context_window=128000, pricing_per_1m_tokens=2.0, strength="Best tool use accuracy"),
            ModelInfo(id="o3", name="OpenAI o3", provider=self.name, context_window=200000, pricing_per_1m_tokens=10.0, strength="Best deep reasoning"),
        ]
