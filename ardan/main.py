...
def run(
    ...
    async def _run():
        if watch:
             ...
             class BuildHandler(FileSystemEventHandler):
                  ...
                  def on_modified(self, event):
                       if not event.is_directory:
                            typer.echo(f"Change detected in {event.src_path}. Re-building...")
             ...
             observer = Observer()
             observer.schedule(BuildHandler(agent, prompt), agent.workspace, recursive=True)
             observer.start()
             try:
        ...
        with console_ui.show_spinner("Building system..."):
            async for update in agent.build_system(prompt, auto=auto):
                ...
                if status == "PLAN_READY":
                    ...
                    if not auto and not Confirm.ask("Proceed?"): return
...
def keys(action: str, provider: str, value: Optional[str] = None):
    ...
    if action == "set":
        credentials_manager.set(provider, value or typer.prompt(f"Enter key for {provider}", hide_input=True))
    elif action == "list":
